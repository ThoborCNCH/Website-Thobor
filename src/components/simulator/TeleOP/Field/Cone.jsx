...
export default function Cone({ position, props }) {
  ...
  useFrame((state) => {
    ...
    bratPosition.setFromMatrixPosition(
      DoamneIartaCeUrmeaza.bratBody.current.matrixWorld
    );
    ////console.log("dada", bratPosition)

    let bratQuaternion = new Quaternion(0, 0, 0, 0);
    bratQuaternion.setFromRotationMatrix(
      DoamneIartaCeUrmeaza.bratBody.current.matrixWorld
    );
    ...
    if (
      dist <= 2 &&
      conePosition.y >= 1.5 &&
      !DoamneIartaCeUrmeaza.controls.f
    ) {
  ...
  useEffect(() => {
    ...
    coneAPICylinder.angularVelocity,
    coneAPICylinder.rotation,
    coneAPICylinder.velocity,
    coneAPICylinder.position,
    position,
  ]);
